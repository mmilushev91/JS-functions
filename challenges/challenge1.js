...
3. Create a method 'displayResults' which displays the poll results. The
 method takes a string as an input (called 'type'), which can be either
  'string' or 'array'. If type is 'array', simply display the results array as it is, using console.log(). 
...
const poll = {
  ...
  registerNewAnswer() {
    const promptLine = [this.question, ...this.options].join('\n');

    let answer;

    while (true) {
      answer = prompt(promptLine);

      if (answer > 0 && answer < 4) {
        break;
      }
      alert('Pick valid option from 0 to 3!');
    }
